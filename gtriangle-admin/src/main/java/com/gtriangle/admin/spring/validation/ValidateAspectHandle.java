package com.gtriangle.admin.spring.validation;
...
public class ValidateAspectHandle {
	...
	@Pointcut("@annotation(com.gtriangle.admin.spring.validation.ValidateGroup)")
				...
				if(an.isModel()){
					...
					for(Object tmpArgs:args){
						if(tmpArgs instanceof HttpServletResponse){
							...
							WebResult.renderJsonDataFail(response, validateEntity.getErrorMessage());
						}
					}
					return "";
