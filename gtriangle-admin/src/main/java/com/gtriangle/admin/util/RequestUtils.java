...
public class RequestUtils {
	...
	/**
	 * 检查访问pc系统的客户端 是否是移动设备，移动设备（除hd，app，pc-native以外）均不能访问
	 * @param request
	 * @return
	 */
	public static boolean checkMobileAccess(HttpServletRequest request) {
		
		String agent = request.getHeader("emp-agent");
	    if(null != agent && !"".equals(agent)){
	    	agent = agent.toLowerCase();
	    	if(agent.indexOf("mobile") > 0) {
	    		Pattern p = Pattern.compile("mtscrm");
	        	boolean isMtApplication = p.matcher(agent).find();
	        	if(isMtApplication == false) {
	        		//response.sendRedirect("http://wap.mtscrm.com/app/download.html");
	        		return true;
	        	}
	    	}
	    }
